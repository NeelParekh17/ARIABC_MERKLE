...
#
# Persistent storage: /home/neel/Desktop/ariabc_cluster (survives reboots, unlike /tmp)
set -euo pipefail
...
# Persistent directories — survive reboots (unlike /tmp which may be cleared at boot)
...
export PROFILE_RUN_GATEWAY_DIAGNOSTIC="${PROFILE_RUN_GATEWAY_DIAGNOSTIC:-0}"
...
BASE_LOG="$ROOT/scripts/bench_full_results/overhead_4node_${BENCH_PRESET}_${RUN_TS}"
mkdir -p "$BASE_LOG"

echo "=== 4-profile overhead benchmark (3 distinct PG machines, persistent paths) ==="
echo "PG nodes: 236 (neel), 248 (neel), 27.54 (neel)"
echo "Gateway : $GW_HOST ($GW_USER)"
echo "Preset  : $BENCH_PRESET"
echo "Repo   : $REMOTE_REPO_ROOT"
echo "Install: $REMOTE_INSTALL_DIR"
echo "Kafka  : $KAFKA_BOOTSTRAP"
echo "Output : $BASE_LOG"

ssh_base=(ssh -i "$SSH_KEY" -o BatchMode=yes -o StrictHostKeyChecking=no -p "$SSH_PORT")

# --------------------------------------------------------------------------
# Bootstrap — create persistent dirs and sync binaries on all nodes
# --------------------------------------------------------------------------
bootstrap_nodes() {
  ...
  for node_user_host in "neel@10.129.148.247" "neel@10.129.148.248" "neel@10.129.148.246"; do
    ...
    # Sync full install dir (includes bin/, lib/, share/ — all required for initdb/postgres)
    rsync -az --delete -e "ssh -i $SSH_KEY -o BatchMode=yes -o StrictHostKeyChecking=no -p $SSH_PORT" \
      /work/ARIABC/install/ "${user_host}:${REMOTE_INSTALL_DIR}/"
    # Sync both server and gateway binaries (probe checks require both on every node)
    rsync -az -e "ssh -i $SSH_KEY -o BatchMode=yes -o StrictHostKeyChecking=no -p $SSH_PORT" \
      /work/ARIABC/AriaBC/ariabc_pg/build/bin/ariabc_pg_server \
      /work/ARIABC/AriaBC/ariabc_pg/build/bin/ariabc_pg_gateway \
      "${user_host}:${REMOTE_REPO_ROOT}/ariabc_pg/build/bin/"
    # Sync scripts and workloads
    rsync -az --exclude='__pycache__' --exclude='*.pyc' --exclude='bench_full_results' \
      -e "ssh -i $SSH_KEY -o BatchMode=yes -o StrictHostKeyChecking=no -p $SSH_PORT" \
      /work/ARIABC/AriaBC/scripts/ "${user_host}:${REMOTE_REPO_ROOT}/scripts/"
  done

  # Gateway (240) also needs gateway binary — already covered by PG node loop above,
  # but explicitly sync gateway binary too
  echo "  Bootstrap: gateway binary -> $GW_USER@$GW_HOST"
  rsync -az -e "ssh -i $SSH_KEY -o BatchMode=yes -o StrictHostKeyChecking=no -p $SSH_PORT" \
    /work/ARIABC/AriaBC/ariabc_pg/build/bin/ariabc_pg_gateway \
    "${GW_USER}@${GW_HOST}:${REMOTE_REPO_ROOT}/ariabc_pg/build/bin/ariabc_pg_gateway"
...
if [[ ! -f "$KAFKA_HOME/bin/kafka-topics.sh" ]]; then
  ...
  if [[ ! -f "$KAFKA_TGZ" ]]; then
    wget -q "https://archive.apache.org/dist/kafka/3.7.0/kafka_2.13-3.7.0.tgz" -O "$KAFKA_TGZ" \
      || curl -sSL "https://archive.apache.org/dist/kafka/3.7.0/kafka_2.13-3.7.0.tgz" -o "$KAFKA_TGZ"
  fi
  tar -xzf "$KAFKA_TGZ"
...
# Ensure advertised.listeners uses external IP so all cluster nodes can reach Kafka
if ! grep -q "^advertised.listeners=PLAINTEXT://\$GW_IP" "\$SERVER_PROPS"; then
  if grep -q "^advertised.listeners" "\$SERVER_PROPS"; then
    sed -i "s|^advertised.listeners=.*|advertised.listeners=PLAINTEXT://\$GW_IP:9092|" "\$SERVER_PROPS"
  else
    echo "advertised.listeners=PLAINTEXT://\$GW_IP:9092" >> "\$SERVER_PROPS"
  fi
fi

if "\$TOPICS_SH" --bootstrap-server "\$GW_IP:9092" --list >/dev/null 2>&1; then
  echo "Kafka already running at \$GW_IP:9092"
  exit 0
fi
...
for i in \$(seq 1 60); do
  if "\$TOPICS_SH" --bootstrap-server "\$GW_IP:9092" --list >/dev/null 2>&1; then
    echo "Kafka broker ready at \$GW_IP:9092 (after \${i}s)"
    exit 0
  fi
  sleep 1
done
echo "ERROR: Kafka broker did not become ready" >&2
exit 1
REMOTE_EOF
)
  "${ssh_base[@]}" "$GW_USER@$GW_HOST" bash <<< "$remote_cmd"
}
...
assert_profile_outputs() {
  ...
  if ! grep -q "Benchmark completed\. Artifacts:" "$log"; then
...
# Sync latest bench script to gateway before each profile run
sync_scripts() {
  rsync -az -e "ssh -i $SSH_KEY -o BatchMode=yes -o StrictHostKeyChecking=no -p $SSH_PORT" \
...
run_one() {
  ...
  /usr/bin/time -p "$ROOT/scripts/distributed/preflight_then_run_full.sh" \
    ...
    --db-conn-pool-cap "$PROFILE_DB_CONN_POOL_CAP" \
    --db-conn-pool-size "$PROFILE_DB_CONN_POOL_SIZE" \
    --det-window "$PROFILE_DET_WINDOW" \
    --postcheck-convergence-timeout-s "$PROFILE_POSTCHECK_CONVERGENCE_TIMEOUT_S" \
    --postcheck-convergence-poll-ms "$PROFILE_POSTCHECK_CONVERGENCE_POLL_MS" \
    --postcheck-convergence-stable-rounds "$PROFILE_POSTCHECK_CONVERGENCE_STABLE_ROUNDS" \
    --comparison-profile "$profile" \
    --kafka-bootstrap "$KAFKA_BOOTSTRAP" \
    |& tee -a "$log" >&2

  local path
  path="$(grep -o 'Local out dir\s*:\s*.*' "$log" | tail -n 1 | sed -E 's/^.*:\s+//' | tr -d '\r')"
  if [[ -z "$path" ]]; then
...
# --------------------------------------------------------------------------
# Bootstrap all nodes with persistent directories and latest binaries
# --------------------------------------------------------------------------
bootstrap_nodes

# --------------------------------------------------------------------------
# Profile 1: vanilla-pg (nondet, no Raft, no Kafka, no gateway)
# --------------------------------------------------------------------------
echo ">>> [1/4] vanilla-pg"
if ! VANILLA_DIR="$(run_single_node_profile_min vanilla-pg nondet)"; then
  echo "ERROR: aborting at profile vanilla-pg" >&2
  exit 1
fi

# --------------------------------------------------------------------------
# Profile 2: base-no-raft-no-kafka (det single-node, no gateway, no kafka)
# --------------------------------------------------------------------------
echo ">>> [2/4] base-no-raft-no-kafka"
if ! BASE_DIR="$(run_single_node_profile_min base-no-raft-no-kafka det)"; then
  echo "ERROR: aborting at profile base-no-raft-no-kafka" >&2
  exit 1
fi
...
if ! KAFKA_ONLY_DIR="$(run_one kafka-only-no-raft 0 "$KAFKA_HOME")"; then
...
export PROFILE_GATEWAY_TIMEOUT_S=120
export ARIABC_MAX_CASE_TIMEOUT_S=180
if ! RK_DIR="$(run_one raft-kafka 0 "$KAFKA_HOME")"; then
  echo "ERROR: aborting at profile raft-kafka" >&2
  exit 1
fi

GATEWAY_DIAG_DIR=""
if [[ "$PROFILE_RUN_GATEWAY_DIAGNOSTIC" == "1" ]]; then
  echo ">>> [diag] gateway-no-raft-no-kafka"
  export PROFILE_WAIT_MAJORITY=0
  export PROFILE_SERVER_BYPASS_RAFT=1
  export PROFILE_GW_BROADCAST_ALL=0
  export PROFILE_CASE_TIMEOUT_S=90
  export PROFILE_GATEWAY_TIMEOUT_S=90
  export ARIABC_MAX_CASE_TIMEOUT_S=90
  if ! GATEWAY_DIAG_DIR="$(run_one raft-no-kafka 1 "" "det")"; then
    echo "ERROR: diagnostic profile gateway-no-raft-no-kafka failed" >&2
    exit 1
  fi
fi
...
# Analysis
# --------------------------------------------------------------------------
PLOT_OUT="$BASE_LOG/four_profile_valid_wall_combined.png"
STRICT_PLOT_OUT="$BASE_LOG/four_profile_valid_strict_wall_combined.png"
CSV_OUT="$BASE_LOG/overhead_comparison.csv"
BEST_CASE_OUT="$BASE_LOG/overhead_comparison_best_case.csv"
...
echo "strict_combined_plot=$STRICT_PLOT_OUT"
echo "overhead_csv=$CSV_OUT"
if [[ -n "$GATEWAY_DIAG_DIR" ]]; then
  echo "gateway_diag_dir=$GATEWAY_DIAG_DIR"
fi
cat "$CSV_OUT"
