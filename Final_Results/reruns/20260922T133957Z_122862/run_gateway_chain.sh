#!/usr/bin/env bash
# Gateway chain: cmds 1-5 from COMMANDS.md
# YCSB → OOM-1 → OOM-2 → TPCC-workers → TPCC-warehouses
# All share neel@10.129.27.111 so must run sequentially.
set -Eeuo pipefail
...
log() { echo "[$(date '+%Y-%m-%dT%H:%M:%S%z')] [gateway-chain] $*"; }

cd "$ROOT"
unset SKIP_SYNC SKIP_BUILD DET_CLIENT_WORKERS || true

# The local ariabc_pg_gateway binary was rebuilt after the cluster manifest was
# written: binary SHA fda270... != manifest SHA 6e9eba... so FORCE_BUILD=1 is
# required for YCSB's first cluster case to rebuild on the nodes and rewrite the
# manifest.  After YCSB finishes the manifest is current and we unset it.
export FORCE_BUILD=1

# ── 1. YCSB ────────────────────────────────────────────────────────────────────
log "=== CMD 1/5: YCSB (FORCE_BUILD=1 — stale manifest) ==="
...
unset FORCE_BUILD  # manifest is now current; no rebuild needed for OOM/TPCC
